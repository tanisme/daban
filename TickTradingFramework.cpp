	...
	m_AdminPassword(AdminPassword)
{
	...
	m_MessageArray = (TTF_Message_t*)calloc(TTF_MESSAGE_ARRAY_SIZE, sizeof(TTF_Message_t));
...
TTF_Message_t* CTickTradingFramework::MessageAllocate()
{
...
//void CTickTradingFramework::OnTime(const boost::system::error_code& error)
//{
//	if (error) {
...
//    //    std::string showSecuritys = "600129,688665,603108,688238,605588,603335,301487,301201,300379,300375,300630,301151,688636,002151,000026,600592,300398";
//    //    Stringsplit(showSecuritys, ',', showVec);
//
//    //    for (auto it : showVec) {
//    //        auto iter = m_mInstruments.find(it);
//    //        if (iter == m_mInstruments.end()) continue;
//    //        ShowOrderBook((char*)it.c_str());
//    //    }
//    //}
//
//	m_timer.expires_from_now(boost::posix_time::milliseconds(60000));
//	m_timer.async_wait(boost::bind(&CTickTradingFramework::OnTime, this, boost::asio::placeholders::error));
//}
...
void CTickTradingFramework::HandleQueueMessage()
{
    ...
    while (true)
    {
        ...
        //m_pool.Free<TTF_Message_t >(pMessage, sizeof(TTF_Message_t));
    }
}
...
void CTickTradingFramework::MDOnRtnOrderDetail(CTORATstpLev2OrderDetailField& OrderDetail)
{
...
void CTickTradingFramework::MDOnRtnTransaction(CTORATstpLev2TransactionField& Transaction)
{
...
void CTickTradingFramework::MDOnRtnNGTSTick(CTORATstpLev2NGTSTickField& Tick)
{
...
void CTickTradingFramework::InitOrderMap() {
...
double CTickTradingFramework::GetOrderNoToPrice(int SecurityIDInt, TTORATstpLongSequenceType OrderNO) {
    auto Price = 0.0;
...
void CTickTradingFramework::AddOrderNoToPrice(int SecurityIDInt, TTORATstpLongSequenceType OrderNO, TTORATstpPriceType Price) {
    m_orderNoToPrice[SecurityIDInt][OrderNO] = Price;
}

void CTickTradingFramework::DelOrderNoPrice(int SecurityIDInt, TTORATstpLongSequenceType OrderNO) {
    auto iter = m_orderNoToPrice.find(SecurityIDInt);
...
void CTickTradingFramework::AddHomebestOrder(int SecurityIDInt, TTORATstpLongSequenceType OrderNO,
...
stHomebestOrder* CTickTradingFramework::GetHomebestOrder(int SecurityIDInt, TTORATstpLongSequenceType OrderNO) {
    auto iter = m_homeBaseOrder.find(SecurityIDInt);
...
void CTickTradingFramework::DelHomebestOrder(int SecurityIDInt, TTORATstpLongSequenceType OrderNO) {
    auto iter = m_homeBaseOrder.find(SecurityIDInt);
...
int CTickTradingFramework::GetHTLPriceIndex(int SecurityIDInt, TTORATstpPriceType Price, TTORATstpTradeBSFlagType Side) {
    TORASTOCKAPI::CTORATstpSecurityField* pInstrument = m_pInstruments + SecurityIDInt;
...
void CTickTradingFramework::AddOrderPriceIndex(int SecurityIDInt, int priceIndex, TTORATstpTradeBSFlagType Side) {
    auto& orderIndex = Side==TORA_TSTP_LSD_Buy?m_buyPriceIndex:m_sellPriceIndex;
    orderIndex[SecurityIDInt].insert(priceIndex);
}

void CTickTradingFramework::DelOrderPriceIndex(int SecurityIDInt, int priceIndex, TTORATstpTradeBSFlagType Side) {
    auto& orderIndex = Side==TORA_TSTP_LSD_Buy?m_buyPriceIndex:m_sellPriceIndex;
...
void CTickTradingFramework::InsertOrderN(int SecurityIDInt, TTORATstpLongSequenceType OrderNO, TTORATstpPriceType Price,
...
int CTickTradingFramework::ModifyOrderN(int SecurityIDInt, TTORATstpLongVolumeType Volume,
...
void CTickTradingFramework::DeleteOrderN(int SecurityIDInt, int priceIndex,
                                TTORATstpLongSequenceType OrderNo, TTORATstpTradeBSFlagType Side) {
...
void CTickTradingFramework::PostPrice(TTORATstpSecurityIDType SecurityID, TTORATstpExchangeIDType ExchangeID) {
...
void CTickTradingFramework::ShowOrderBook(TTORATstpSecurityIDType SecurityID) {
    int SecurityIDInt = atoi(SecurityID);
    int count = 0;
    {
...
int CTickTradingFramework::FindPriceIndexOrderNo(std::vector<stOrder *> &orderNOs,
